...
class FunAsLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.param = self.add_weight(name='param', shape=(1290,), initializer=tf.keras.initializers.RandomNormal(mean=0.,stddev=0.2), trainable=True)
...
if __name__ == "__main__":
    ...
    all_max_mode_amps = np.loadtxt('new_PBCO_max_bound_vectors.txt', dtype=np.float32 , delimiter=',')
    max_mode_amps = np.loadtxt('new_PBCO_mode_amp_bounds_ver3.txt', dtype=np.float32 , delimiter=',')
    included_mode = pd.read_csv('exc_idx_list.csv', header=None).values.flatten().tolist()
    this_included_mode = included_mode[slurm_array_task_id]
    max_mode_amps[this_included_mode] = all_max_mode_amps[this_included_mode]
